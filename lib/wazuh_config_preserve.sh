...
# Override certificate generation so upstream Wazuh beta5 config files survive.
generate_wazuh_certs_locally() {
  ...
  [[ -f "$idx_dir/internal_users.yml" ]] || die "Expected beta5 config file missing before certificate generation: $idx_dir/internal_users.yml"
  [[ -f "$idx_dir/wazuh.indexer.yml" ]] || die "Expected beta5 config file missing before certificate generation: $idx_dir/wazuh.indexer.yml"
  [[ -f "$dash_dir/wazuh.yml" ]] || die "Expected beta5 dashboard config missing before certificate generation: $dash_dir/wazuh.yml"

  rm -rf "$ca_dir"
  rm -rf "$idx_dir/certs" "$idx_dir/private" "$mgr_dir/certs" "$mgr_dir/private" "$dash_dir/certs" "$dash_dir/private"
...
EOF
  ...
  chown "${WAZUH_INDEXER_UID}:${WAZUH_INDEXER_GID}" "$idx_dir/certs/wazuh.indexer-key.pem" "$idx_dir/certs/admin-key.pem"
  ...
  [[ -f "$idx_dir/internal_users.yml" ]] || die "Certificate generation deleted internal_users.yml"
  [[ -f "$idx_dir/wazuh.indexer.yml" ]] || die "Certificate generation deleted wazuh.indexer.yml"
  [[ -f "$dash_dir/wazuh.yml" ]] || die "Certificate generation deleted dashboard wazuh.yml"
  ...
  ok "Fresh Wazuh TLS certificates generated without deleting beta5 configuration files"
}
