...
@test "Wazuh API port is configured in manager api.yaml and generated dashboard plugin wazuh.yml" {
  ...
  [ "$status" -eq 0 ]
  run grep -F 'Generating Wazuh dashboard plugin wazuh.yml' "$WAZUH_API_OVERRIDE"
  [ "$status" -eq 0 ]
  ...
  [ "$status" -ne 0 ]
  run grep -F 'source "$SCRIPT_DIR/lib/wazuh_dashboard_api_override.sh"' "$REPO_ROOT/lib/wazuh_v3_certs.sh"
  [ "$status" -eq 0 ]
